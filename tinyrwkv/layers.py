...
class Int8Linear:
  def __init__(self, in_features: int, out_features: int, bias=False):
    ...
    self.scale = Tensor.empty(out_features)
  def __call__(self, x: Tensor) -> Tensor: return x.linear(self.weight.cast(dtypes.default_float).T * self.scale)
  ...
  def quantize(state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    for k, v in state_dict.items():
      if ".key" in k or ".value" in k or ".receptance" in k or ".gate" in k or ".output" in k or ".value" in k:
        scale = (v.abs().max(axis=1) / 127.0).cast(dtypes.default_float)
        new_state_dict[k] = (v.T / scale).T.round().cast(dtypes.int8)
...
def NF4Linear(block_size):
  ...
  class _NF4Linear:
    def __init__(self, in_features: int, out_features: int, bias=False):
      ...
      self.scale = Tensor.empty(int(out_features * in_features / block_size), 1)

    def __call__(self, x: Tensor) -> Tensor:
      low_bits = (self.weight << 4).contiguous()
      unpacked = Tensor.stack(self.weight, low_bits, dim=-1) >> 4
    ...
    def quantize(state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
      ...
      for k, v in state_dict.items():
        if ".key" in k or ".value" in k or ".receptance" in k or ".gate" in k or ".output" in k or ".value" in k:
          ...
          new_state_dict[k.replace(".weight", ".scale")] = scale.cast(dtypes.default_float)
        else:
          new_state_dict[k] = v
      return new_state_dict
  return _NF4Linear
